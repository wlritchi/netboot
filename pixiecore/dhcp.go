...
func (s *Server) serveDHCP(conn *dhcp4.Conn) error {
	for {
		...
		if fwtype == FirmwarePixiecoreIpxe {
			s.logBootStage("DHCP", pkt.HardwareAddr, fwtype, machineStateProxyDHCPIpxe, "Client")
		} else {
			s.logBootStage("DHCP", pkt.HardwareAddr, fwtype, machineStateProxyDHCP, "Client")
		}
...
func (s *Server) validateDHCP(pkt *dhcp4.Packet) (mach Machine, fwtype Firmware, err error) {
	...
	case 7, 9:
		// Type 7 and 9 are both x64 EFI variants (standards disagree on details)
		// Both accept the same x64 EFI binary
		mach.Arch = ArchX64
		fwtype = FirmwareEFI64
	case 11, 19:
		// Type 11 and 19 are both ARM64 EFI variants (regular and HTTP boot)
		// Both accept the same ARM64 EFI binary
		mach.Arch = ArchARM64
